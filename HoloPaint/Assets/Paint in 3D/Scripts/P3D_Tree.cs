...
public class P3D_Tree
{
	...
	public void SetMesh(Mesh newMesh, int newMaterialIndex = 0, bool forceUpdate = false)
	{
		if (newMesh != null)
		{
			...
			if (forceUpdate == false && newMesh == mesh && newMaterialIndex == materialIndex && newMesh.vertexCount == vertexCount)
			{
				return;
			}
	...
	public List<P3D_Result> FindNearest(Vector3 point, float maxDistance)
	{
		ClearResults();
	...
	public List<P3D_Result> FindBetweenNearest(Vector3 startPoint, Vector3 endPoint)
	{
		ClearResults();
		...
		if (IsReady == true)
		{
			...
			if (bestTriangle != null)
			{
				AddToResults(bestTriangle, bestWeights, bestDistance01);
			}
		}
