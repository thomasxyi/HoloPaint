...
{
	public static P3D_Painter.PaintOperation AdditiveBlend(Color color, Texture2D shape, Texture2D detail, Vector2 detailScale)
	{
#if UNITY_EDITOR
				...
				for (var x = rect.XMin; x < rect.XMax; x++)
				{
					for (var y = rect.YMin; y < rect.YMax; y++)
					{
						if (IsInsideShape(matrix, x, y, ref shapeCoord) == true)
						{
							...
							canvas.SetPixel(x, y, AdditiveBlend(old, add));
						}
					}
				}
			}
